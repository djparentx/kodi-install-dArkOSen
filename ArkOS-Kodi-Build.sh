...
git ls-remote https://github.com/xbmc/xbmc.git --h --sort origin "refs/heads/*" | cut -d "/" -f3 | grep "Nexus\|Matrix\|Omega"
  echo ""
  exit 1
fi
...
GBM_MODIFIERS_FLAG=""
if [[ "${KODI_DISABLE_GBM_MODIFIERS}" == "1" ]]; then
  GBM_MODIFIERS_FLAG="-DGBM_HAS_MODIFIERS=OFF"
fi
cmake ../kodi-source -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=/home/kodi/bin-kodi -DENABLE_INTERNAL_FLATBUFFERS=ON -DENABLE_INTERNAL_FMT=ON -DENABLE_INTERNAL_SPDLOG=ON -DENABLE_INTERNAL_DAV1D=ON -DENABLE_INTERNAL_PCRE=ON -DCORE_PLATFORM_NAME=gbm -DAPP_RENDER_SYSTEM=gles ${GBM_MODIFIERS_FLAG}
if [ $? != 0 ]; then
  echo ""
